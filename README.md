...
- через фабрику, создавая объект в соответствии с количеством входных параметров (одно значение - окружность, три значения - треугольник);
- объявлением объекта соответствующего типа используя классы напрямую (Circle и Triangle);
- при помощи методов класса;


## Установка и запуск проекта
Для запуска проекта необходимо установленный **Visual Studio**
...
При использовании ссылочных типов, при указании параметров, создается их копия в объекте.
...
Алгоритм классов построен по принципу хранения вычисленного значения для объекта сразу после его объявления или изменения значений. Таким образом, не происходит повторное вычисление площади, при последующих обращениях к полю `Area`.<br>
...
`double figure.UpdateArea(double radius)` - Для объекта Circle. Вызывает исключение в Triangle<br>
`double figure.UpdateArea(double sideA, double sideB, double sideC)` - Для объекта Triangle. Вызывает исключение в Circle<br>
...
Метод для определения прямоугольного треугольника:
`bool figure.Rectangular()` - возвращает `true` если фигура прямоугольная.
...
Базовая последовательность использования методов для объекта, при создании фигуры через конструктор и расчета площади через статические методы класса.<br>
![Sequence diagram](https://i.ibb.co/ctLDZPf/Untitled-Diagram-7.jpg)


## Расширение проекта
Возможно наращивание библиотеки, добавляя собственные фигуры. Для этого необходимо объявить класс с фигурой и связать этот класс в интерфейсах FigureFactory и IFigure.
